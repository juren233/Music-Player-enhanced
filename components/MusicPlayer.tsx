...
export const MusicPlayer = React.memo<MusicPlayerProps>(({
    ...
    // Theme Colors - Apple Music style
    const transitionClass = "transition-[color,background-color,border-color,opacity,shadow,transform,filter] duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]";
    ...
    const glassBg = isDarkMode ? 'bg-neutral-900/70 border-white/[0.06]' : 'bg-white/75 border-black/[0.04]';

    // Control Buttons Logic
    const controlBtnClass = `p-2.5 rounded-xl relative ${transitionClass} ${iconHoverClass} active:scale-90`;
    const shuffleActiveClass = isShuffle
        ? (isDarkMode ? 'bg-white/10 text-white shadow-[0_0_12px_rgba(255,255,255,0.08)]' : 'bg-black/8 text-black shadow-[0_0_12px_rgba(0,0,0,0.06)]')
        : textDimColor;

    // Active Color - Apple Music Red for PROGRESS BAR
    const activeBarColor = 'bg-[#FA2D48]';
    const activeShadow = 'shadow-[0_0_12px_rgba(250,45,72,0.5)]';

    return (
        <div className="w-full h-[100px] relative z-50">
            <div className={`absolute inset-0 backdrop-blur-3xl border-t ${transitionClass} ${glassBg}`} />
            ...
            <div className="relative h-full max-w-screen-2xl mx-auto px-6 flex items-center justify-between gap-8">
                ...
                <div className="flex-1 flex flex-col items-center justify-center max-w-[600px]">
                    ...
                    <div className="flex items-center gap-8 mb-3">
                        ...
                        {/* Play/Pause Button - Hero Element - Apple Music style */}
                        <button
                            onClick={onPlayPause}
                            className={`relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-105 active:scale-95 ${isDarkMode
                                ? 'bg-white text-black shadow-[0_4px_20px_rgba(255,255,255,0.25)]'
                                : 'bg-[#1d1d1f] text-white shadow-[0_4px_20px_rgba(0,0,0,0.2)]'
                                }`}
                        >
                            ...
                            <div className="relative w-7 h-7 flex items-center justify-center">
                                ...
                                <Play className={`absolute w-full h-full fill-current ml-0.5 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${!isPlaying ? 'scale-100 rotate-0 opacity-100' : 'scale-50 rotate-90 opacity-0'}`} />
                            </div>
                        </button>
